...
export default function Notification() {
  ...
  const fetchFollowRequests = async () => {
    ...
    try {
      const response = await fetch(`http://${API_BASE_URL}/api/user/${currentUser._id}/followRequests`);
      if (!response.ok) {
  ...
  const handleAccept = async (requesterId) => {
    try {
      const response = await fetch(`http://${API_BASE_URL}/api/user/${currentUser._id}/followRequest/accept`, {
        method: "POST",
  ...
  const handleReject = async (requesterId) => {
    try {
      const response = await fetch(`http://${API_BASE_URL}/api/user/${currentUser._id}/followRequest/reject`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ requesterId: requesterId.toString() }),
      });
  ...
  const handleFollowBack = async (requesterId) => {
    try {
      const response = await fetch(`http://${API_BASE_URL}/api/user/${currentUser._id}/followBack`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ requesterId: requesterId.toString() }),
      });
